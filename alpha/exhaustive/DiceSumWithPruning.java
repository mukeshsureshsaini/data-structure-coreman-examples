...
public class DiceSumWithPruning {

	public static void diceSum(int numberOfDices, int desiredSum) {

		diceSum(numberOfDices, 0, desiredSum, new ArrayList<>());
	}

	public static void diceSum(int numberOfDice, int currentSum, int desiredSum, List<Integer> choicesSoFar) {
		if (numberOfDice == 0) {
		...
		// why && ,why not || becuae we must stay in range too low even we make maximum
		// value choice && too high even we make min value choice
			...
			for (int i = 1; i <= 6; i++) {
				...
				choicesSoFar.add(i);
				diceSum(numberOfDice - 1, currentSum + i, desiredSum, choicesSoFar);
